...
let hasGravity = true;
let collisionedInLastFrame = {};

function drawImage() {
  class Ball {
    constructor(id, x, y, color, velocityX) {
      this.id = id;
      this.x = x;
      this.y = y;
      this.color = color;
      ...
      this.velocityY = hasGravity ? 0 : Math.random() * 20;
    }
    draw() {
      ctx.beginPath();
    ...
    update() {
      ...
      if (this.y + this.size > window.innerHeight && this.velocityY >= 0) {
        ...
        this.velocityY = -this.velocityY + (hasGravity ? 1 : 0);
    ...
    for (let i = 0; i < ballsArray.length; i++) {
      for (let j = i + 1; j < ballsArray.length; j++) {
        ...
        if (bSq < minBSq) {
          ...
          if (hasCollisions) {
            console.log(
              collisionedInLastFrame,
              !!(
                (collisionedInLastFrame[b1.id] &&
                  collisionedInLastFrame[b1.id].indexOf(b2.id) > -1) ||
                (collisionedInLastFrame[b2.id] &&
                  collisionedInLastFrame[b2.id].indexOf(b1.id) > -1)
              )
            );
            if (
              (collisionedInLastFrame[b1.id] &&
            ...
            b2.x += separationX;
            b2.y += separationY;
            if (collisionedInLastFrame[b1.id]) {
              collisionedInLastFrame[b1.id].push(b2.id);
            } else {
              collisionedInLastFrame[b1.id] = [b2.id];
            }
  ...
  function animate(animatedRunning) {
    if (animatedRunning === animatedRunningCurrent) {
      ...
      if (hasExplosions || hasCollisions) checkBallsCollision();
    }
  }
  init();
  ...
  document.getElementById("Ok").addEventListener("click", () => {
    ...
    hasCollisions = document.getElementById("Collisions").checked;
    hasGravity = document.getElementById("Gravity").checked;
    restart();
  });
  ...
  document.getElementById("Cancel").addEventListener("click", () => {
    ...
    document.getElementById("Collisions").checked = hasCollisions;
    document.getElementById("Gravity").checked = hasGravity;
  });
}
